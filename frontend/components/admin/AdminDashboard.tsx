...
import { BarChart, Bar, XAxis, YAxis, Tooltip, ResponsiveContainer, Cell, CartesianGrid, PieChart, Pie, AreaChart, Area, Legend } from 'recharts';
import { Play, Pause, AlertTriangle, Smartphone, Users, RotateCcw, FileSpreadsheet, Search, ChevronDown, ChevronRight, Plus, Trash2, Edit2, Save, X, LayoutDashboard, Trophy, Activity, FolderPlus, Download, Clock, Calendar, FileText } from 'lucide-react';

// Modern Color Palette for Charts
const COLORS = ['#6366f1', '#8b5cf6', '#ec4899', '#f43f5e', '#f97316', '#eab308', '#22c55e', '#06b6d4', '#3b82f6'];

export const AdminDashboard: React.FC = () => {
    const [activeTab, setActiveTab] = useState<'dashboard' | 'teams' | 'users' | 'settings'>('dashboard');
    ...
    const [teamForm, setTeamForm] = useState({ name: '', description: '', imageUrl: '' });

    // Config State
    const [configForm, setConfigForm] = useState<{ startTime: string, endTime: string }>({ startTime: '', endTime: '' });
    ...
    async function refreshData(){
        try {
            ...
            if (d && d.config && !data) {
                setConfigForm({
                    startTime: d.config.startTime ? new Date(d.config.startTime).toISOString().slice(0, 16) : '',
                    endTime: d.config.endTime ? new Date(d.config.endTime).toISOString().slice(0, 16) : ''
                });
            }
        } catch (e) {
    ...
    data?.users?.filter(u => u.lastVotedAt).forEach(u => {
        const dept = u.dept.split(' ').map(w => w[0]).join(''); // Abbreviate Dept Name
    ...
    const toggleVoting = async () => {
        ...
        const now = new Date();
        const start = data.config.startTime ? new Date(data.config.startTime) : null;
        ...
        if (!data.config.isVotingOpen) {
            ...
            if (start && end) {
                if (now < start || now > end) {
                    alert("Cannot start voting: Current time is outside the configured Start/End window. Please adjust the times in Settings.");
                    return;
                }
            }
        }
    ...
    const handleRevoke = async (e: React.FormEvent) => {
        e.preventDefault();
        const user = data.users.find(u => u.email === revokeQuery || u.rollNo === revokeQuery);
        if (!user) {
    ...
    const handleDetailedReport = () => {
        ...
        data.users.forEach(u => {
            ...
            Object.entries(u.votes).forEach(([teamId, count]) => {
                const teamName = data.teams.find(t => t.id === teamId)?.name || 'Unknown Team';
                const row = [
                    `"${u.name}"`,
                    `"${u.rollNo}"`,
                    `"${u.dept}"`,
        ...
        link.href = url;
        link.setAttribute('download', `CWC_Raw_Data_${new Date().toISOString().slice(0, 10)}.csv`);
    ...
    const handleSummaryReport = () => {
        ...
        const totalVotes: any = Object.values(data.teamVotes).reduce((a: number, b: number) => a + b, 0);
    ...
    // Stats constants
    const totalVotesCast = Object.values(data.teamVotes).reduce((a: number, b: number) => a + b, 0);
      ...
      `}</style>
            ...
            <div className="admin-glass p-2 rounded-2xl flex flex-col md:flex-row justify-between items-center gap-4 sticky top-4 z-30 mx-0.5 no-print ring-1 ring-white/5">
                ...
                <div className="flex items-center gap-4 px-3 w-full md:w-auto justify-between md:justify-end">
                    <div className={`flex items-center gap-2 px-4 py-1.5 rounded-full border ${data.config.isVotingOpen ? 'bg-green-500/10 border-green-500/20' : 'bg-red-500/10 border-red-500/20'}`}>
                        <div className={`w-2 h-2 rounded-full ${data.config.isVotingOpen ? 'bg-green-500 animate-pulse' : 'bg-red-500'}`} />
                        <span className={`text-xs font-bold uppercase tracking-wider ${data.config.isVotingOpen ? 'text-green-400' : 'text-red-400'}`}>
                    ...
                    >
                        {data.config.isVotingOpen ? <><Pause size={16} className="mr-2" /> Pause Voting</> : <><Play size={16} className="mr-2" /> Start Session</>}
                    </Button>
                </div>
            </div>
            ...
            {activeTab === 'dashboard' && (
                <div className="space-y-6">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
                        {[
                            { label: 'Total Votes', value: totalVotesCast, icon: Activity, color: 'text-indigo-400', bg: 'from-indigo-500/10' },
                            { label: 'Registered Students', value: data.users.length, icon: Users, color: 'text-blue-400', bg: 'from-blue-500/10' },
                            { label: 'Turnout Today', value: `${Math.round((activeUsers / (data.users.length || 1)) * 100)}%`, icon: Clock, color: 'text-emerald-400', bg: 'from-emerald-500/10' },
                        ].map((stat, idx) => (
                            <div key={idx} className={`bg-slate-800 rounded-2xl p-6 border border-slate-700/50 bg-gradient-to-br ${stat.bg} to-transparent relative overflow-hidden group`}>
                    ...
                    <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 h-auto lg:h-[450px]">
                        ...
                        <div className="lg:col-span-2 admin-glass rounded-2xl p-6 flex flex-col">
                            <div className="flex justify-between items-center mb-6">
                                ...
                                <div className="flex gap-2">
                                    <Button onClick={handleSummaryReport} variant="secondary" className="px-3 py-1 text-xs h-8 bg-slate-700">
                                        <FileText size={14} className="mr-1" /> Vote List (PDF)
                                    </Button>
                                    <span className="text-xs text-slate-500 font-mono flex items-center">UPDATED: {new Date().toLocaleTimeString()}</span>
                                </div>
                            </div>
                            <div className="flex-1 w-full min-h-[300px]">
                                <ResponsiveContainer width="100%" height="100%">
                                    <BarChart data={chartData} layout="vertical" margin={{ top: 0, right: 30, left: 40, bottom: 0 }}>
                                        ...
                                        <Tooltip cursor={{ fill: 'rgba(255,255,255,0.03)' }} contentStyle={{ backgroundColor: '#0f172a', borderColor: '#334155', borderRadius: '8px', color: '#fff' }} />
                        ...
                        <div className="flex flex-col gap-6 h-full">
                            ...
                            <div className="admin-glass rounded-2xl p-6 flex-1 min-h-[200px] flex flex-col">
                                <h3 className="text-sm font-bold text-slate-300 uppercase mb-4">Department Activity</h3>
                                <div className="flex-1 relative">
                                    ...
                                    <div className="absolute inset-0 flex items-center justify-center pointer-events-none">
                                        <div className="text-center">
                                            <span className="text-2xl font-bold text-white block">{activeUsers}</span>
                                            <span className="text-[10px] text-slate-500 uppercase">Voters</span>
                                        </div>
                                    </div>
                                </div>
                            </div>
                            ...
                            {/* Voting Trend */}
                            <div className="admin-glass rounded-2xl p-6 flex-1 min-h-[200px] flex flex-col">
                                <h3 className="text-sm font-bold text-slate-300 uppercase mb-4">Traffic (24H)</h3>
                ...
                <div className="space-y-6">
                    <div className="flex justify-between items-center bg-slate-800 p-6 rounded-2xl border border-slate-700 shadow-lg">
                        <div>
                            ...
                            <p className="text-sm text-slate-400">Configure voting options and candidates.</p>
                        </div>
                        {!isAddingTeam && (
                            <Button onClick={() => { setIsAddingTeam(true); setEditingTeam(null); setTeamForm({ name: '', description: '', imageUrl: '' }); }}>
                    ...
                    {isAddingTeam && (
                        <div className="admin-glass rounded-2xl p-8 border-l-4 border-l-indigo-500 animate-in slide-in-from-top-4 fade-in duration-300">
                            <div className="flex justify-between items-center mb-6">
                                <h3 className="text-xl font-bold text-white">{editingTeam ? 'Edit Team Details' : 'Register New Team'}</h3>
                                <button onClick={() => setIsAddingTeam(false)} className="text-slate-400 hover:text-white transition-colors"><X /></button>
                            </div>
                            <form onSubmit={handleSaveTeam} className="grid grid-cols-1 md:grid-cols-2 gap-6">
                                <div className="md:col-span-2">
                                    ...
                                    <input className="w-full bg-slate-900 border border-slate-700 text-white rounded-xl px-4 py-3 focus:ring-2 focus:ring-indigo-500 focus:outline-none transition-all"
                                        value={teamForm.name} onChange={e => setTeamForm({ ...teamForm, name: e.target.value })} required placeholder="e.g. Code Warriors" />
                                </div>
                                <div className="md:col-span-2">
                                    <label className="block text-xs font-bold text-slate-400 uppercase tracking-wider mb-2">Description (Slogan)</label>
                                    <input className="w-full bg-slate-900 border border-slate-700 text-white rounded-xl px-4 py-3 focus:ring-2 focus:ring-indigo-500 focus:outline-none transition-all"
                                        value={teamForm.description} onChange={e => setTeamForm({ ...teamForm, description: e.target.value })} required placeholder="Short catchphrase..." />
                                </div>
                                <div className="md:col-span-2">
                                    <label className="block text-xs font-bold text-slate-400 uppercase tracking-wider mb-2">Image URL</label>
                                    <input className="w-full bg-slate-900 border border-slate-700 text-white rounded-xl px-4 py-3 focus:ring-2 focus:ring-indigo-500 focus:outline-none transition-all"
                                        value={teamForm.imageUrl} onChange={e => setTeamForm({ ...teamForm, imageUrl: e.target.value })} required placeholder="https://..." />
                                </div>
                                <div className="md:col-span-2 flex justify-end gap-3 mt-4">
                                    <Button type="button" variant="secondary" onClick={() => setIsAddingTeam(false)}>Cancel</Button>
                                    <Button type="submit">
                                        <Save className="w-4 h-4 mr-2" /> {editingTeam ? 'Update' : 'Create'}
                                    </Button>
                                </div>
                            </form>
                        </div>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                        {data.teams.length === 0 && !isAddingTeam && (
                            <div className="col-span-full py-20 text-center text-slate-500 bg-slate-800/30 rounded-3xl border-2 border-dashed border-slate-700">
                                ...
                                <h3 className="text-xl font-bold text-slate-300">No Teams Configured</h3>
                                <p className="text-sm max-w-sm mx-auto mt-2">The voting roster is empty. Click "Add Team" above to setup the candidates.</p>
                            </div>
                        )}
                        {data.teams.map(team => (
                            <div key={team.id} className="bg-slate-800 rounded-2xl overflow-hidden border border-slate-700 shadow-lg group relative hover:border-indigo-500/50 transition-all duration-300 hover:shadow-2xl hover:-translate-y-1">
                                <div className="aspect-video w-full relative bg-slate-900 overflow-hidden">
                                    <img src={team.imageUrl} alt={team.name} className="w-full h-full object-cover opacity-80 group-hover:opacity-100 group-hover:scale-110 transition-all duration-500" />
                                    <div className="absolute inset-0 bg-gradient-to-t from-slate-900 via-transparent to-transparent opacity-90" />

                                    {/* Action Overlay */}
                                    <div className="absolute top-2 right-2 flex gap-2 opacity-0 group-hover:opacity-100 transition-opacity duration-300">
                                        <button onClick={() => startEditTeam(team)} className="p-2 bg-white/10 backdrop-blur-md rounded-lg hover:bg-white/20 text-white"><Edit2 size={16} /></button>
                                        <button onClick={() => handleDeleteTeam(team.id)} className="p-2 bg-red-500/80 backdrop-blur-md rounded-lg hover:bg-red-500 text-white"><Trash2 size={16} /></button>
                                    </div>

                                    <div className="absolute bottom-4 left-4 right-4">
                                        <h4 className="font-bold text-xl text-white truncate">{team.name}</h4>
                                        <p className="text-xs text-slate-300 line-clamp-1">{team.description}</p>
                                    </div>
                                </div>
                                <div className="p-4 bg-slate-800 border-t border-slate-700">
                                    <div className="flex items-center justify-between text-xs font-bold">
                                        <span className="text-slate-500 uppercase tracking-widest">Total Votes</span>
                                        <span className="text-indigo-400 bg-indigo-500/10 px-3 py-1 rounded-full border border-indigo-500/20">{data.teamVotes[team.id] || 0}</span>
                                    </div>
                                </div>
                            </div>
                        ))}
            ...
            {activeTab === 'users' && (
                <div className="space-y-6">
                    ...
                    <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 bg-slate-800 p-6 rounded-2xl border border-slate-700 shadow-lg no-print">
                        <div>
                            <h3 className="text-2xl font-bold text-white">Student Registry</h3>
                            <p className="text-sm text-slate-400">View participation logs and generate reports.</p>
                        </div>
                        <div className="flex gap-3">
                            <Button onClick={handleDetailedReport} className="bg-emerald-600 hover:bg-emerald-500 text-white shadow-lg shadow-emerald-500/20">
                                        ...
                                        <React.Fragment key={user.id}>
                                            ...
                                            >
                                                ...
                                                <td className="p-5">
                                                    ...
                                                    <div className="text-xs font-mono text-indigo-400 mt-0.5">{user.rollNo}</div>
                                                </td>
                                                <td className="p-5 text-slate-400 font-medium max-w-[200px] truncate">{user.dept}</td>
                                                <td className="p-5 text-center">
                                                    <span className={`inline-flex items-center justify-center min-w-[32px] h-8 rounded-lg text-xs font-bold ${Object.keys(user.votes).length > 0 ? 'bg-indigo-500/20 text-indigo-300 border border-indigo-500/30' : 'bg-slate-700 text-slate-500'}`}>
                                                        {Object.values(user.votes).reduce((a: number, b: number) => a + b, 0)}
                                                    </span>
                                                </td>
                                                <td className="p-5 text-right text-slate-500 text-xs font-mono">
                                                    {user.lastVotedAt ? new Date(user.lastVotedAt).toLocaleString() : '---'}
                                                </td>
                                            </tr>
                                                ...
                                                <tr className="bg-slate-900/40">
                                                    <td colSpan={5} className="p-6">
                                                        <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
                                                            {/* Vote History */}
                                                            <div>
                                                                <h4 className="text-xs font-bold text-slate-500 uppercase tracking-widest mb-3">Vote Breakdown</h4>
                                                                <div className="bg-slate-800 rounded-xl border border-slate-700 p-4 space-y-2">
                                                                    {Object.keys(user.votes).length > 0 ? Object.entries(user.votes).map(([tid, count]) => {
                                                                        const team = data.teams.find(t => t.id === tid);
                                                                        return (
                                                                            <div key={tid} className="flex justify-between items-center p-2 rounded-lg bg-slate-700/30">
                                                                                <div className="flex items-center gap-3">
                                                                                    {team?.imageUrl && <img src={team.imageUrl} className="w-8 h-8 rounded-md object-cover" alt="" />}
                                                                                    <span className="text-slate-200 text-sm font-medium">{team?.name || 'Unknown Team'}</span>
                                                                                </div>
                                                                                <span className="font-bold text-white bg-slate-900 px-3 py-1 rounded-md text-xs">{count} Votes</span>
                                                                            </div>
                                                                        );
                                                                    }) : <p className="text-slate-500 text-sm italic">No votes cast yet.</p>}
                                                                </div>
                                                            </div>

                                                            {/* User Details */}
                                                            <div>
                                                                <h4 className="text-xs font-bold text-slate-500 uppercase tracking-widest mb-3">Contact Profile</h4>
                                                                <div className="bg-slate-800 rounded-xl border border-slate-700 p-4 text-sm space-y-3">
                                                                    ...
                                                                    <div className="flex justify-between border-b border-slate-700 pb-2">
                                                                        ...
                                                                        <span className="text-slate-200 font-medium">{user.year} / {user.gender}</span>
                                                                    </div>
                                                                    <div className="flex justify-between pt-1">
                                                                        <span className="text-slate-500">Device ID Hash</span>
                                                                        <span className="text-indigo-400 font-mono text-xs">{user.boundDeviceId || 'Unbound'}</span>
                                                                    </div>
                                                                </div>
                                                            </div>
                                                        </div>
                ...
                <div className="max-w-3xl mx-auto">
                    <div className="admin-glass rounded-2xl p-8">
                        <div className="flex items-center gap-3 mb-8 border-b border-slate-700 pb-4">
                            <div className="p-3 bg-indigo-500/20 rounded-xl"><Clock className="text-indigo-500" /></div>
                            <div>
                                <h3 className="text-xl font-bold text-white">Voting Schedule</h3>
                                <p className="text-sm text-slate-400">Strictly enforce voting hours. Students cannot vote outside this window.</p>
                            </div>
                        </div>
                        ...
                        <form onSubmit={saveConfig} className="space-y-6">
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                                <div>
                                    <label className="block text-xs font-bold text-slate-400 uppercase tracking-wider mb-2">Session Start Time</label>
                                    <div className="relative">
                                ...
                                <div>
                                    <label className="block text-xs font-bold text-slate-400 uppercase tracking-wider mb-2">Session End Time</label>
                                    <div className="relative">
                                        <Calendar className="absolute left-4 top-1/2 -translate-y-1/2 text-slate-500 w-5 h-5" />
                                        <input
                                            type="datetime-local"
                            ...
                            <div className="bg-indigo-500/10 border border-indigo-500/20 rounded-xl p-4 flex gap-4 items-start">
                                ...
                                <div className="text-sm text-indigo-300">
                                    <strong>Note:</strong> The "Start Session" button on the dashboard will only work if the current time falls within this configured window. If no window is set, manual toggling works freely.
                                </div>
                            </div>

                            <div className="flex justify-end pt-4">
                                <Button type="submit" className="px-8 py-3 text-lg">Save Schedule</Button>
                            </div>
                        </form>
                    </div>
                </div>
